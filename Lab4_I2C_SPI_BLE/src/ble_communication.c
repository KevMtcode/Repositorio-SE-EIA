...
static const char *TAG = "PanelHMI";
...
//Definir la identidad del servicio:
...
void nus_send_response(const char *msg) //Envía texto al celular
{
    if (conn_handle == BLE_HS_CONN_HANDLE_NONE || !notify_enabled) { //Verifica que haya cliente conectado y suscrito
        ESP_LOGW(TAG, "Cliente no conectado o no suscrito a TX Notify");
        return;
    }

    struct os_mbuf *om = ble_hs_mbuf_from_flat(msg, strlen(msg)); //Crea un buffer 
    if (om == NULL) {
...
static int nus_rx_access_cb(uint16_t conn_handle_param, //Enviar datos del celular a la ESP32
                            ...
                            void *arg)
{
    uint8_t data[17]; //Hasta 17 caracteres admite
    uint16_t len = 0;

    int rc = ble_hs_mbuf_to_flat(ctxt->om, data, sizeof(data) - 1, &len); //sizeof(data) - 1 para leer de data[n], máximo n - 1  
...
static const struct ble_gatt_svc_def gatt_svr_svcs[] = { //Declara la estructura con las características del servicio
    {
...
static int ble_gap_event_cb(struct ble_gap_event *event, void *arg) //Manejo de eventos:
{
    switch (event->type) {
...
static void ble_app_advertise(void) //Advertising
{
